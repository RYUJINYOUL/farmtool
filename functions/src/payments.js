...
// 클라이언트에서 리디렉션을 처리하므로, 아래 Secret은 더 이상 사용하지 않습니다.
...
exports.confirmPayment = onRequest(
  ...
  async (req, res) => {
    ...
    await cors(req, res, async () => {
      // POST 요청만 허용
      if (req.method !== 'POST') {
        return res.status(405).json({ code: 'METHOD_NOT_ALLOWED', message: 'POST 메소드만 허용됩니다.' });
      }

      // 쿼리 파라미터가 아닌 요청 본문(body)에서 정보 전달 받기
      const { paymentKey, orderId, amount, collectionName, subscriptionPeriodInMonths } = req.body;
      ...
      if (!paymentKey || !orderId || !amount || !collectionName || !subscriptionPeriodInMonths) {
        console.error('Missing required body parameters for payment confirmation.');
