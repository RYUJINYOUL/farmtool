...
import { HouseDongCodes, LegalDongItem } from '@/lib/housePmsApi';
...
export default function PermitSearchControls({
  ...
  useEffect(() => {
    const getDongs = async () => {
      ...
      if (tempMainRegion && tempMainRegion !== '전국' && tempSubRegion && tempSubRegion !== '시군구 선택') {
        ...
        try {
          const { data, error } = await HouseDongCodes(tempMainRegion, tempSubRegion);
  ...
  const applySearchParams = (
    ...
    router.push(`/house/?${newSearchParams.toString()}`);
  };
