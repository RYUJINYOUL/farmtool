...
import PostDetailWithQuotation from '@/components/middle/equipment/Quotation';
...
const Page = (props) => {
  ...
  const toggleConfirmed = async () => {
    try {
      const docRef = doc(db, 'equipApply', id);
            ...
            <div className={`space-y-8 ${message.imageDownloadUrls.length === 0 ? "" : "pt-6" }`}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-gray-50/50 rounded-xl p-6 space-y-1">
                  <dt className="text-sm font-medium text-gray-500">경력사항</dt>
                  <dd className="text-base text-gray-900">{message.constructionExperience || "-"}</dd>
                </div>
                
                <div className="bg-gray-50/50 rounded-xl p-6 space-y-1">
                  <dt className="text-sm font-medium text-gray-500">장비이용료</dt>
                  <dd className="text-base text-gray-900">{message.rentalRates || "-"}</dd>
                </div>

                <div className="bg-gray-50/50 rounded-xl p-6 space-y-1">
                  <dt className="text-sm font-medium text-gray-500">필요장비</dt>
                  <dd className="text-base text-gray-900">{(message.SubCategories || []).join(", ") || "-"}</dd>
                </div>

                <div className="bg-gray-50/50 rounded-xl p-6 space-y-1">
                  <dt className="text-sm font-medium text-gray-500">운전원필요여부</dt>
                  <dd className="text-base text-gray-900">{message.contactPerson || "-"}</dd>
                </div>

                <div className="bg-gray-50/50 rounded-xl p-6 space-y-1">
                  <dt className="text-sm font-medium text-gray-500">공사주소</dt>
