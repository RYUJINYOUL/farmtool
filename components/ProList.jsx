    ...
    <div className='w-full h-full'>
      {messages.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {messages.map(({ address, businessLicense, SubCategories, phoneNumber, constructionExperience, contactPerson, createdDate, id, companyName, favorites, imageDownloadUrls}, idx) => {
            // 각 항목별로 현재 사용자가 찜했는지 여부 확인
            const isWishListed = currentUser?.uid && favorites.includes(currentUser.uid);

            return (
              <div key={id} // key는 id를 사용하는 것이 더 안정적입니다.
