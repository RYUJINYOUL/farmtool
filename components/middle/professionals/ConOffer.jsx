...
const ConOffer = ({ // <-- 이름 변경 및 searchParams 대신 직접 props 받기
  ...
  const toggleFavorite = useCallback(async (itemId, currentFavorites) => {
    ...
    try {
      const constructionDocRef = doc(db, 'proApply', itemId);
  ...
  const fetchMessages = useCallback(async (isInitialLoad) => {
    ...
    let baseQueryRef = collection(db, "proApply");
    let queryConditions = [];
  ...
  const onClickCard = ({ id }) => {
    router.push(`/professionals/apply/${id}`);
  };
     ...
     <div className='w-full h-full'>
          {messages.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {messages.map(({ address, imageDownloadUrls, createdDate, confirmed, career, rates, 
              favorites, companyName, phoneNumber, description, id, userKey, certificate}, idx) => { 
