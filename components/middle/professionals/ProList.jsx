...
const ProList = ({ // <-- 이름 변경 및 searchParams 대신 직접 props 받기
  ...
  const toggleFavorite = useCallback(async (itemId, currentFavorites) => {
    ...
    try {
      const constructionDocRef = doc(db, 'professionals', itemId);
  ...
  const fetchMessages = useCallback(async (isInitialLoad) => {
    ...
    let baseQueryRef = collection(db, "professionals");
    let queryConditions = [];
  ...
  const onClickCard = ({ id }) => {
     router.push(`/professionals/registration/${id}`);
  };
            ...
            return (
              <div key={idx} // key는 id를 사용하는 것이 더 안정적입니다.
                ...
                <div className="space-y-3 text-sm text-gray-600">
                  ...
                  <div className="flex justify-between">
                    <span className="text-gray-500">전문전공:</span>
